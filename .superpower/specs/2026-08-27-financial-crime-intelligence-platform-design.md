...
Every recovery checkpoint MUST have a compact immutable checkpoint manifest naming owner and creation cadence; partition and `state_generation_id`; covered per-source coordinate vector and exclusive event-time frontier equal to the Section 12.3 partition state watermark; schema, feature, graph-state, and replay versions; canonical state hash excluding any later unresolved or partially applied member/state; immutable object location, byte length, and SHA-256; retention/expiry; read-back integrity-check result; and latest restore-drill result. Creation MUST verify that every affecting command through the covered vector/frontier belongs to a complete group with a durable `STATE_APPLIED` receipt, upload the immutable checkpoint, read it back, verify its bytes/hash and canonical state hash, and only then commit its coverage in the PostgreSQL continuation ledger. Restore MUST verify the manifest, object hash, versions, group-consistent coverage, source-coordinate vector, and canonical state hash before applying it, then replay strictly after—not at or before—each covered source coordinate.
