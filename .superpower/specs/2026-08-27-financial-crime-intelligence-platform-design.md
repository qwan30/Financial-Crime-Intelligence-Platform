...
A scoring component MUST be declared in the frozen package as either **partition-local**, depending only on state whose entity-command partition covers the decision cutoff, or **cross-entity**, depending on counterpart, neighborhood, graph, or other multi-partition state. Each entity-command partition MUST derive its input frontier from the replayed per-normalized-source coordinate vector and the exclusive event-time frontier carried by the latest causally valid source-progress watermark or idle-partition advancement record for each source. Its committed state watermark is the greatest resolved cutoff strictly below its exclusive event-time frontier for which every command capable of affecting the partition at or before the cutoff has resolved, in the same `state_generation_id`, into a complete-group `STATE_APPLIED` receipt. It MUST NOT skip an unresolved ordering gap, partial group, `REPLAY_REQUIRED` group, or cross-generation command. A component completeness watermark is the minimum such verified coverage across all inputs it requires in that generation, and every committed state or component watermark cutoff MUST remain strictly below the applicable exclusive event-time frontier. Neither watermark depends on downstream scoring, evidence, sinks, or `COMPLETE`, and consumer-local clocks or silence MUST NOT advance either watermark.

The PostgreSQL continuation and coverage ledger MUST store the source-coordinate vector, exclusive event-time frontier, `state_generation_id`, component/input ID, group/member receipt IDs, and coverage status. Every scoring attempt MUST pin and read the exact same-generation state version and hash for each required entity/partition at its decision cutoff; later state advancement MUST NOT change those inputs. An idle entity inherits its entity-command partition's verified coverage while retaining the unchanged pinned state version/hash. A cross-entity component MUST wait until every mandatory counterpart and graph input has the required same-generation/cutoff `STATE_APPLIED` coverage; cross-generation inputs are prohibited. Missing mandatory coverage blocks and retries rather than creating a wall-clock-dependent score. Only a mandatory input proven deterministically terminally absent after required group/coverage processing may yield `FAILED_CLOSED` with a typed terminal-absence reason; optional-component absence may be `ABSTAINED` only under the frozen abstention policy.
...
| Per-command/group continuation, eligibility, source-coordinate frontiers, and committed entity/completeness watermarks | PostgreSQL durable command/group receipts, `EventTransitionResult`, outbox/receipts, and coverage ledger | Redis applied-transition marker and Kafka consumer offset |
...
- `EntityStateCommand`;
- `EntityPartitionControlRecord` (`SOURCE_PROGRESS` or `SOURCE_IDLE`);
- `RiskDecisionEvent`;
- `EvidencePackage`;
- `CaseLifecycleEvent`;
- `AuditEvent`.

Every event MUST include `schema_version`, stable `event_id`, source, event time, ingest time, producer, producer Git SHA, trace ID, and payload hash. Every `EntityStateCommand` MUST additionally bind the canonical expected member IDs, count, and set hash; group ID/hash; `state_generation_id` and `replay_run_id`; normalized-source topic/partition/offset and payload hash; event cutoff; affected entity ID; and deterministic transition ID. Monetary values MUST use decimal amount plus ISO currency, never binary floating point. Unknown required enum values MUST fail validation; additive optional fields MAY be accepted under the compatibility policy.

### 11.4 Primary flow

The required local flow is:

`source replay -> raw Kafka topic -> validation/quarantine -> normalized event -> atomic per-entity command/control fan-out -> durable per-member eligibility -> entity-partition-owned causal feature/state update -> scoring attempt -> evidence package -> risk event -> case/outbox persistence -> investigation projections -> analyst workflow`.

The normalized-event consumer is only the validator, deterministic group enumerator, and Kafka fan-out producer. It MUST atomically publish exactly one `EntityStateCommand` for each distinct affected entity plus the required source-progress/idle control records; it MUST NOT create or apply an entity transition or mutate Redis. The current entity-command partition owner is the sole serialization and mutation authority for every entity keyed to that partition.
...
- entity-partition control records: explicitly assigned to each entity-command partition, preserving normalized-source coordinates;
- risk decisions: scored `subject_id`;
- case lifecycle events: `case_id`;
- quarantine and late-event records: original `event_id`.

The normalized consumer MUST validate the normalized event, canonicalize the distinct affected-entity set, and use one Kafka transaction to publish the complete command group and any causally following control records before committing the normalized-source offset. Partial fan-out is forbidden. All mutations for one entity MUST route to the same entity-command partition, whose current consumer-group owner is the sole serialization and mutation authority for that entity. Partition count, key skew, hot keys, replication, retention, and consumer-group ownership MUST be recorded in the benchmark manifest. Cross-entity subgraph state is eventually consistent and MUST expose its completeness watermark.
...
- Commands within the allowed-lateness window MUST be durably ordered by each entity owner before state application.
- A group found late for any expected member during Section 12.3 eligibility MUST take its whole-group `REPLAY_REQUIRED` disposition and MUST NOT silently mutate current state.
...
The normalized-event consumer MUST deterministically enumerate the complete distinct affected-entity set before publishing anything. `transition_group_id` MUST derive from the normalized event ID, payload hash, `state_generation_id`, `replay_run_id`, and entity-command contract version. In one Kafka transaction it MUST publish exactly one `EntityStateCommand` per expected entity and the causally following source-progress records, then commit the consumed normalized-source offset. Every command MUST carry the exact canonical expected member set/count/hash, group ID/hash, lineage key/hash, normalized-source coordinates and payload hash, event cutoff, affected entity ID, and deterministic transition ID. Transaction abort leaves neither visible commands nor a committed normalized-source offset; transaction commit makes the complete fan-out and offset advancement visible atomically. The normalized-event consumer MUST NOT create an `EventTransitionResult`, decide lateness from entity state, apply a transition, or read/write Redis.

On command delivery, the entity-command partition owner MUST validate the command and its set/count/hash, place it in that entity's durable total order by `(event_time, event_id)`, and persist or verify an immutable member-continuation and eligibility record before committing the entity-command Kafka offset. That record MUST include the member's cutoff comparison against the pinned committed state watermark and its typed eligibility reason under the frozen lateness policy. Eligibility inspection MUST NOT mutate Redis. A later admitted command for the same entity MUST NOT cross an unresolved earlier command. Workers MUST continue consuming and durably admitting commands while earlier work waits; they MUST NOT block Kafka consumption on group completion, scoring, sinks, or Redis application, subject to the bounded-backpressure rule in Section 12.7.

After and only after PostgreSQL contains exactly one matching eligibility record for every expected member and no extra or conflicting member, one atomic group transaction MUST verify the canonical member set/count/hash and choose exactly one disposition: `APPLY_ALLOWED`, `REPLAY_REQUIRED`, or terminal quarantine. Any member late under the frozen policy makes the whole group `REPLAY_REQUIRED`; any deterministically malformed, unsupported, or conflicting member makes the whole group terminal-quarantined; either disposition creates no live `EventTransitionResult` and causes zero member Redis mutations. The receipt MUST bind group/event/payload, every member/partition eligibility and reason, lineage, source coordinates, cutoff, policy/classifier versions, and affected replay partitions. A `REPLAY_REQUIRED` gap continues to block live state-watermark advancement until controlled replay/reconciliation resolves it in a candidate generation under Section 12.4.

For an `APPLY_ALLOWED` group, PostgreSQL reconciliation MUST release each member only when it is the earliest unresolved command for that entity and the source-authoritative input frontier proves that no earlier `(event_time, event_id)` command can still arrive. The owning entity-command partition worker then atomically creates or verifies that member's durable `EventTransitionResult`, computes the transition from the exact prior state version/hash, and is the sole authority permitted to compare-and-set the Redis projection and persist its applied marker/receipt. Each result MUST be unique by `transition_id` and contain the group receipt ID/hash; expected set/count/hash; `state_generation_id`; `replay_run_id`; lineage key/hash; input/payload hash; source topic/partition/offset; cutoff; prior and next state versions/hashes; complete deterministic transition bytes or a read-back-verified content-addressed blob; and monotonic processing stage. After every expected member has a verified applied receipt, one atomic PostgreSQL transaction MUST advance the complete group to `STATE_APPLIED`. Transition bytes and receipts are immutable; processing stage MAY advance only by compare-and-set or append-only child records. PostgreSQL is the processing-continuation and watermark source of truth; Redis is a rebuildable projection and a Kafka offset is only a consumption cursor.
...
Each normalized source partition MUST publish replayable, source-authoritative control records into every entity-command partition. A `SOURCE_PROGRESS` record names the next-uncovered/exclusive source coordinate `N` and an exclusive event-time frontier and is valid only when it is causally after complete atomic fan-out of every source record with coordinate strictly less than `N`; coverage includes exactly coordinates strictly before that coordinate. A `SOURCE_IDLE` record is an explicit upstream assertion, with epoch and valid-through/revocation source coordinates, that the source will emit no event earlier than its exclusive frontier; this is the only idle-partition advancement contract. Resume MUST publish a higher-epoch progress record before later commands.

The canonical frontier/vector ownership is scoped to `state_generation_id`; `replay_run_id` on a control receipt records the producing run but MUST NOT create a second canonical vector within that generation. Before any `EntityPartitionControlRecord` changes the replayed vector or its control record's entity-command Kafka offset commits, the partition owner MUST atomically persist or verify an immutable control continuation/vector receipt. The receipt MUST contain control ID; control type, source coordinate/vector, exclusive event-time frontier; idle epoch and valid-through/revocation coordinates when applicable; generation/run lineage, payload/hash, and prior/resulting vector/hash. A duplicate control with the same ID and payload hash MUST verify the immutable receipt and resulting vector/hash, then commit the offset if still pending without advancing the vector twice. The same control ID with a different payload hash MUST persist a typed conflict child receipt linked to the original receipt, leave the vector/watermark unchanged, and only then commit that record's offset; failure to persist/verify that conflict receipt or any stored vector-hash mismatch MUST fail closed. Retry MUST load the receipt, compare the stored prior vector/hash, and idempotently verify or complete the resulting-vector compare-and-set before offset commit. Receipt persistence or vector compare-and-set failure MUST leave the offset uncommitted and MUST NOT advance the input frontier, state watermark, or Redis. The committed entity-command cursor MAY advance only through a contiguous partition prefix in which every command and control record has its required durable receipt.

Entity partitions MUST replay the verified receipts to maintain a per-source next-uncovered coordinate vector; gaps, regressions, revoked/superseded idle assertions, or controls from a different lineage do not advance canonical coverage except through the activation-token-authorized candidate-to-live handoff in Section 12.4. Consumer-local timers, observed silence, poll timeouts, and wall-clock heuristics MUST NOT create source progress or canonical completeness.

For each entity-command partition, the input frontier is the replayed per-source coordinate vector plus the minimum causally proven exclusive event-time frontier across its required normalized sources. The partition state watermark is the greatest cutoff strictly below its exclusive event-time frontier for which every command affecting the partition at or before the cutoff is in the same generation and belongs to a complete group with durable `STATE_APPLIED`. It MUST stop before any missing source coordinate, unresolved order gap, partial group, `REPLAY_REQUIRED` group, or cross-generation work. An entity with no affecting command through a verified frontier inherits the partition coverage with its prior state version/hash unchanged. State-watermark advancement and scoring MUST pin those exact versions/hashes; `COMPLETE` remains downstream of scoring and is never a state-watermark prerequisite.

A retry or reconciler MUST load the command/group continuation by group/member ID, `state_generation_id`, and `replay_run_id`, verify the stored expected set/count/hash and immutable payloads, and resume from its durable stage. If Redis already shows a member's applied lineage marker and next-state hash, the owner MUST NOT apply it again; it verifies and records the missing receipt. Missing eligibility, order predecessors, member receipts, or complete-group `STATE_APPLIED` block state application or watermark advancement, not Kafka offset commit after durable admission. After `STATE_APPLIED`, retry MUST reconstruct any missing decision/evidence artifact or sink message from immutable lineage inputs, then resume delivery, receipt recording, and `COMPLETE`. A missing/hash-invalid payload or different Redis version/hash MUST fail closed and trigger reconciliation or replay. Wall-clock timeout or partial group arrival MUST NOT become a canonical `ABSTAINED`/`FAILED_CLOSED` result, and an event MUST NOT be marked fully processed solely because Redis state advanced.

Crash recovery MUST follow these boundaries:

- before the normalized Kafka transaction commits: neither any command/control fan-out nor the normalized-source offset is visible;
- after a control record arrives but before its control receipt persists: the control redelivers, its entity-command offset remains uncommitted, and the source vector/watermark remain unchanged;
- after the control receipt/vector compare-and-set but before its entity-command offset commits: redelivery verifies the immutable receipt and resulting vector/hash, then commits the offset without advancing the vector twice;
- after atomic fan-out but before a member eligibility receipt: the entity command redelivers and Redis remains unchanged;
- after durable member continuation/eligibility but before the entity-command offset commit: redelivery verifies the receipt, commits the offset, and does not mutate Redis;
- after exact-set `APPLY_ALLOWED` but before all member applications: reconciliation releases each ordered member to its owning partition, which applies only its missing stored transition;
...
- after `COMPLETE`: retries verify the complete record without repeating effects; no source or entity-command offset waits for this stage.
...
Checkpoint and materialization coverage at each sealed barrier MUST equal the partition state watermark defined in Section 12.3, including its exact next-uncovered source-coordinate vector and exclusive event-time frontier, with coverage only through coordinates strictly below each vector component and the covered cutoff strictly below that exclusive frontier, and MUST exclude every later unresolved, replay-required, cross-generation, or partially applied member/state. Each affected partition MUST replay rebuilt `G2` state through that barrier and verify staged corrections. Each untouched partition MUST be deterministically materialized/copied into the `G2` namespace from only its fenced, verified `G1` checkpoint/state at that group-consistent head, then read-back-verified with its source and `G2` hashes. No partition may mutate after its sealed barrier until global activation succeeds or aborts.

Only after every partition exists and is verified in `G2` may one atomic PostgreSQL/control transaction compare-and-set the single global active-generation pointer exactly once from expected `G1` to `G2`. The transaction MUST verify a complete partition inventory/map; every partition's rebuild/materialization mode, sealed barrier/head, group-consistent checkpoint/coverage, and `G1`/`G2` state hashes; absence of post-barrier mutation; and that every transition group with any begun or member-applied transition touching any partition at or below a sealed barrier has its complete expected member set and one durable group `STATE_APPLIED` receipt across the complete partition map; and that no required cross-entity input spans generations. It MUST also append an activation token bound to `G2` and use it to authorize the generation-scoped correction outbox and control-frontier handoff. If any partition cannot fence, materialize, or verify, activation MUST fail closed, `G1` remains active, all staged `G2` artifacts remain non-current, and every fence resumes `G1`. External scoring MUST NOT resolve `G2` until the complete inventory is present and MUST NOT observe or assemble a mixed-generation window.

The global activation transaction MUST append an immutable `ActivationRecord` containing manifest/core hashes, candidate/prior generation, replay run, complete partition map with rebuild/materialization modes, all barrier/head offsets, prior and candidate state hashes/versions/checkpoints, buffered-event ranges/hashes, activation token, actor/time, and immutable pre-activation staged-verification receipts: candidate hash/coverage, materialization read-back, no-post-barrier-mutation, cross-entity generation check, conflict checks, and staged artifact counts/hashes. In that same transaction, the activation token MUST atomically seed the live control continuation for every `G2` partition from the verified candidate next-uncovered vector/hash and exclusive frontier, recording a unique handoff receipt bound to `state_generation_id`, candidate producing `replay_run_id`, target `replay_run_id = live`, partition, predecessor vector/hash, frontier, and activation token. The first live control record MUST reference that seeded predecessor and activation token; a missing or mismatched reference MUST fail closed without advancing the vector or offset.

The candidate-to-live frontier handoff MUST be idempotent and crash-safe: retry with the same activation token and identical seed verifies the existing handoff receipt as a no-op, while a different seed/token conflicts and fails closed. A crash after activation commit but before the first live control leaves the durable seed authoritative; a crash after the first live control receipt follows Section 12.3 receipt/offset recovery. Only the activation transaction may authorize the candidate-run-to-live transition within the same activated `state_generation_id`; every cross-generation or unauthorized run transition MUST be rejected. Correction consumers MUST require a valid activation token, apply only artifacts for the globally activated generation, and do so idempotently; only then may they change case/evidence current views, priority, or dispositions. Post-activation drain and correction consumers MUST append application receipts linked by activation token and generation ID; those receipts are not required before the `ActivationRecord` commits. Completion is a derived status only when all required post-activation receipts exist.
...
Every recovery checkpoint MUST have a compact immutable checkpoint manifest naming owner and creation cadence; partition and `state_generation_id`; covered per-source next-uncovered coordinate vector, exclusive event-time frontier, and state-watermark cutoff strictly below that frontier; schema, feature, graph-state, and replay versions; canonical state hash excluding any later unresolved or partially applied member/state; immutable object location, byte length, and SHA-256; retention/expiry; read-back integrity-check result; and latest restore-drill result. Creation MUST verify every control receipt establishing the covered next-uncovered vector and every affecting command through the covered cutoff, including complete-group durable `STATE_APPLIED` receipts, upload the immutable checkpoint, read it back, verify its bytes/hash and canonical state hash, and only then commit its coverage in the PostgreSQL continuation ledger. Restore MUST verify the manifest, object hash, versions, control receipts, group-consistent coverage, source-coordinate vector, and canonical state hash before applying it; because each component `N` is next-uncovered, restore MUST resume inclusively at that next-uncovered coordinate `N`, not after it.
...
- Persistence failure MUST not acknowledge an entity-command offset before its durable member continuation/eligibility exists; normalized-source offset acknowledgement requires the complete atomic Kafka fan-out transaction.
- A control-record receipt persistence/verification or vector compare-and-set failure MUST follow the Section 12.3 control retry rule and MUST NOT acknowledge its entity-command offset or advance coverage.
- Only malformed, unsupported, or conflicting input deterministically proven terminal by exact-set eligibility before creation of `EventTransitionResult`, Redis advancement, or any sink effect may enter terminal quarantine. Its durable group receipt MUST contain source coordinates, payload hash, complete expected set/count/hash, every member reason, classifier version, and proof that no transition or sink effect began.
...
- Backpressure MUST pause new Kafka admission before bounded durable continuation capacity is exceeded; waiting for eligibility, ordering predecessors, state application, scoring, or sinks MUST use durable work queues rather than holding consumer polls open.
...
- `EvidencePackage` and `CaseEvidenceSnapshot` artifacts MUST be append-only; corrections create a superseding version and retain the original.
...
Each case evidence version MUST be an immutable `CaseEvidenceSnapshot` containing `case_evidence_snapshot_id` and `case_id`; prior snapshot ID/hash or genesis; normalized `EvidencePackage` ID/hash references in canonical order; current, superseded, and retracted occurrence mappings; analyst-supplied evidence references and note/comment references in fields distinguishable from source and derived facts; merge/split source and result provenance; completeness and degradation state; creation command ID/type; and canonical content hash. `case_version` is lifecycle/concurrency metadata and MUST NOT be a snapshot canonical field. Sensitivity summaries MAY support display, but authorization MUST be evaluated per underlying evidence item and remains authoritative.

The snapshot hash MUST use Section 14.3 canonicalization over every field above except its own ID/hash; `case_evidence_snapshot_id` MUST be derived from that hash. Only commands that change evidence membership or another canonical evidence field create a successor `CaseEvidenceSnapshot`; lifecycle-only commands MUST reuse the same snapshot ID/hash even when they increment `case_version`. A successor never mutates or deletes a prior snapshot.
...
|---|---|---|---|
| `CREATE_CASE` | no case | case-ingestion service principal | `NEW`; originating `risk_decision_id` as the deterministic ingestion/idempotency key, `evidence_package_id`/hash, exact resulting `CaseEvidenceSnapshot` ID/hash, and `If-None-Match: *` or an equivalent typed absence precondition; prior case version and snapshot ID/hash are canonical null/genesis |
...
| `REOPEN_CASE` | any three closed states or `ESCALATED` | Reviewer | `REOPENED`; new-evidence or quality-review reason and the current snapshot, or a successor snapshot if the command changes a canonical evidence field |

There is no direct `NEW`/`TRIAGED` to closed transition, and an Analyst MUST NOT approve a disposition. Every command MUST pass server-side role, object, assignment, sensitivity, and state authorization; append an atomic audit event; and record actor, role, command, timestamp, previous/new state, reason, prior and resulting case versions, prior and resulting snapshot IDs/hashes, and lineage IDs where present. Commands against existing cases MUST supply the last observed `case_version` and exact `CaseEvidenceSnapshot` ID/hash. `CREATE_CASE` MUST first check its deterministic `risk_decision_id` and canonical payload before evaluating the absence precondition; an identical key and canonical payload MUST return the original case result idempotently, while the same key with a different canonical payload MUST conflict/quarantine. `CREATE_CASE` MUST otherwise use `If-None-Match: *` or an equivalent typed absence precondition and its deterministic ingestion/idempotency key; its audit record MUST use canonical null/genesis for the prior case version and snapshot ID/hash. Lifecycle history MUST reference the exact snapshot observed, reused, or created by each command. `CREATE_CASE` stores the originating package and resulting snapshot; `ATTACH_RISK_OCCURRENCE` records the occurrence and successor snapshot; assignment, triage, start, disposition, and reviewer commands reuse and record the observed snapshot unless the same command changes a canonical evidence field; note/evidence commands create a successor snapshot; merge/split commands record every source and resulting snapshot; disposition proposals reference the immutable snapshot reviewed; and `REOPEN_CASE` references the exact current-or-successor snapshot, plus the approved reopen request when present. Invalid state/version or stale snapshot MUST return `409 Conflict`; only a non-identical competing creation that fails the absence precondition MUST return `409 Conflict`; denied authority MUST return `403 Forbidden`; neither outcome may mutate the case. Only Reviewer `REOPEN_CASE` may change an approved terminal case or `ESCALATED` case to `REOPENED`, which MUST pass through `START_INVESTIGATION` before another disposition proposal.
...
Every mutable case command against an existing case MUST supply the last observed `case_version` through `If-Match` or an equivalent typed field. A stale command MUST return `409 Conflict` with the current version and MUST NOT overwrite another analyst’s work.
...
- events from two different normalized source partitions sharing one entity, adversarially interleaved, proving one entity-command owner preserves `(event_time, event_id)` order and deterministic state/hash;
- source-progress watermark and idle-partition advancement replay, including idle/resume, an unresolved earlier command, and a missing source-coordinate gap, proving no timer-based or gap-skipping state-watermark advancement and unchanged pinned state for an idle entity;
- independent recovery at the normalized-source offset and entity-command offset boundaries, proving atomic complete fan-out on the former and durable continuation/eligibility without Redis mutation on the latter;
- control-receipt/offset recovery before and after immutable receipt/vector persistence, including duplicate and conflicting controls, proving deterministic replay, exactly one vector advance, and no offset or watermark advancement on persistence failure;
- boundary-N restore from a checkpoint whose next-uncovered source coordinate is `N`, proving coordinates below `N` are not replayed and coordinate `N` is replayed inclusively exactly once;
- activation-token-authorized candidate-to-live frontier handoff, including crashes before the first live control and after its receipt, proving exact candidate vector/hash seeding, first-live predecessor validation, idempotent retry, and rejection of cross-generation or unauthorized run transitions;
...
- a two-entity event late for only one member proving exact-set eligibility yields one whole-group `REPLAY_REQUIRED` disposition, zero live member mutations, no later same-entity crossing, and deterministic replay/reconciliation;
...
- case creation replay-first idempotency, proving a deterministic risk-decision/payload check before the absence precondition; an identical create retry returns the original result idempotently; a conflicting create key/payload is quarantined; and only a non-identical competing creation whose absence precondition fails returns `409 Conflict` without mutation;
- case snapshot identity proving lifecycle-only commands reuse the same `CaseEvidenceSnapshot` ID/hash while evidence-membership or other canonical-evidence changes create a successor, including current-versus-successor reopen behavior;
- duplicate/conflicting/late event behavior;
- crashes before/after atomic command fan-out, member eligibility persistence, entity-command offset commit, exact-set disposition, Redis compare-and-set, member receipt, complete-group `STATE_APPLIED`, scoring/outbox, and `COMPLETE`;
