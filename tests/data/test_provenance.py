...
@pytest.mark.parametrize("bad_hash", ["not-a-hash", "A" * 64, "123", "   "])
def test_source_manifest_rejects_invalid_schema_sha256(bad_hash: str) -> None:
    with pytest.raises(ValidationError):
        SourceManifest(
            source_id="amlsim-20k-fanin200",
...
@pytest.mark.parametrize("bad_hash", ["not-a-hash", "A" * 64, "123", "   "])
def test_derived_artifact_manifest_rejects_invalid_hashes(bad_hash: str) -> None:
    with pytest.raises(ValidationError):
        DerivedArtifactManifest(
            source_id="amlsim-20k-fanin200",
            parent_raw_sha256=bad_hash,
            adapter_name="AMLSimSampleAdapter",
            adapter_version="1.0",
            conversion_parameters=(("prefix", "sample_"),),
            output_sha256="b" * 64,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("key", "val", "extra"),),  # type: ignore[arg-type]
    ],
)
def test_derived_artifact_manifest_rejects_invalid_conversion_parameters(
    bad_params: Any,
) -> None:
    with pytest.raises(ValidationError):
        DerivedArtifactManifest(
            ...
            adapter_name="AMLSimSampleAdapter",
            adapter_version="1.0",
            conversion_parameters=bad_params,
            output_sha256="b" * 64,
            row_count=100,
            public_columns=("edge_id", "source_id", "target_id", "amount", "event_time"),
        )
...
def test_sha256_file_failure_redacts_path(tmp_path: Path) -> None:
    non_existent = tmp_path / "secret_directory_99" / "secret_file.bin"
    with pytest.raises(OSError) as exc_info:
        sha256_file(non_existent)
    assert "secret_directory_99" not in str(exc_info.value)
    assert "secret_file.bin" not in str(exc_info.value)
