...
def test_clean_splits_pass_audit() -> None:
    records = {
        ...
        "temporal": CaseRecord("temporal", "scatter_gather", 14, frozenset({"e"}), frozenset({"e4"})),
...
def test_all_non_train_tracks_are_audited_against_train() -> None:
    ...
    for field_name, case_id in manifest_fields:
        records = {
            "train": CaseRecord("train", "fan_in", 100, frozenset({"shared_entity"}), frozenset({"e_train"})),
            case_id: CaseRecord(case_id, "cycle", 200, frozenset({"shared_entity"}), frozenset({"e_non_train"})),
        }
        kwargs = {
            "train_case_ids": ("train",),
            "validation_case_ids": (),
